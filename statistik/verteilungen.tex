...
\begin{itemize}
    \item \(\mu\)=Erwartungswert (z.B. \(0.3\))
    \item \textbf{Wahrscheinlichkeit:} \(P(X=k)=\frac{\mu^k}{k!}e^{-\mu}\); \(k=\) Anzahl der gewünschten Ereignisse
    \item \textbf{Erwartungswert:} \(E(X)=\mu\)
    \item \textbf{Varianz:} \(Var(X)=\mu\)
    \item \textbf{Normalapproximation:} \(X\sim N(\mu, \mu)\)
    \item \textbf{Poisson-Verteilung als Grenzwert der Binomialverteilung:} \(Poi(\mu)=\lim_{n\rightarrow\infty}Bin(n,\frac{\mu}{n})\)
\end{itemize}
